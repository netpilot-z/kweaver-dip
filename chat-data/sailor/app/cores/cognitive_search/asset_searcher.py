...
from app.cores.cognitive_search.graph_func import (get_kgotl, graph_analysis_formview, find_idx_list_of_dict,
                                                   graph_analysis_general, get_kgotl_dip_new)
...
from app.cores.cognitive_search.search_model import GraphFilterParamsModel, AssetSearchParamsDIP
...
class AssetSearcher:
    def __init__(self, request, search_params: AssetSearchParamsDIP, search_configs: SearchConfigs):
        self.request = request
        self.headers = {"Authorization": request.headers.get('Authorization')}
        self.search_params = search_params
        self.search_configs = search_configs

    async def combine_rst_of_lexical_and_vector_search(self, drop_indices_lexical, drop_indices_vec, hits_vec, hits_id_lexical,
    ...
    async def graph_search(self, data_type, hits, properties_alias, entity_types, search_params, request, graph_filter_params,
                           data_params):
        # 5 图分析服务部分
        ...
        # 非场景分析版本
            ...
            re_limit_qa = int(self.search_configs.sailor_search_qa_cites_num_limit)
            ...
            # if (data_type == "resource" and self.search_configs.direct_qa == 'false'
            #         and self.search_configs.sailor_search_if_auth_in_find_data_qa == '0'
            #         and self.search_configs.sailor_search_if_history_qa_enhance == '0'
            #         and self.search_configs.sailor_search_if_kecc == '1'):
    ...
    async def run_func(self, search_params, request, file_path, data_type):
        """
        ...
        if hits_vec:
            for i, hit in enumerate(hits_vec):
                ...
                if hit["_id"] in hits_key_id:
                    # 如果向量搜索结果的id在关键词搜索结果中存在，则将向量搜索结果的分数加到关键词搜索结果的分数中
                    hit['vec_score'] = hit['_score'] / 2
                    lexical_score = vid_hits_key[hit["_id"]]['_score']
                    # 将向量搜索的分数折半后和关键词搜索的分数相加，作为新的关键词搜索分数
                    vid_hits_key[hit["_id"]]['_score'] = hit['vec_score'] + lexical_score
                    ...
                    all_hits.append(hit)
            for value in vid_hits_key.values():  # 关键词搜索结果中，向量搜索结果中没有的，加入 all_hits
                all_hits.append(value)
        else:
            all_hits = hits_key
        # logger.debug(f'before drop_indices: (all_hits) = {all_hits}')
        ...
        logger.info(f"认知搜索总耗时 {total_time_cost} 秒")
        return output

    # # 分析问答型搜索的托底算法
    async def run_func_for_qa(self,search_params, request, file_path, data_type, search_configs):
        """
        ...
        # 获取图谱信息和向量化
        task_get_kgotl = asyncio.create_task(
            get_kgotl(search_params=search_params, output=output, query=query, kgotl_config_file_path=file_path,
        ...
        # data_params 时相关参数， 比如搜索结果上限、实体权重、同义词actrie对象、停用词列表对象等
        # graph_filter_params 是筛选项， 调用 get_kgotl( )时，传入的时初始化的筛选项， 在调用时，会更新筛选项
        (queries, query_cuts, all_syns, entity_types, properties_alias, properties_types, entity2prop,
         vector_index_filed,
         data_params, graph_filter_params) = await task_get_kgotl
    ...
    async def run_func_for_qa_dip(self,search_params, request, file_path, data_type, search_configs):
        """
        认知搜索-列表页主函数，run_func_resource()和 run_func_catalog()都调用这个函数，
        根据资源版或目录版传入的不同参数来执行
        ...
        # 获取图谱信息和向量化
        task_get_kgotl = asyncio.create_task(
            get_kgotl_dip(search_params=search_params, output=output, query=query, kgotl_config_file_path=file_path,
                          graph_filter_params=GraphFilterParamsModel()))
    ...
    async def run_func_resource(self, request, search_params):
        '''数据资源版认知搜索列表的算法入口函数'''
        # file_path是本地配置文件路径
        # base_path = os.path.dirname(os.path.abspath(__file__))
        # file_path = os.path.join(base_path, "search_config/config_search_resource.json")

        output = await self.run_func(
            search_params=search_params,
            request=request,
    ...
    async def run_func_catalog(self, request, search_params):
        '''数据目录版认知搜索列表的算法入口函数'''
        # base_path = os.path.dirname(os.path.abspath(__file__))
    ...
    # 分析问答型搜索的托底算法
    # async def run_func_catalog_for_qa(self,request, search_params, search_configs):
    #     '''数据目录版认知搜索列表的算法入口函数'''
    #     # base_path = os.path.dirname(os.path.abspath(__file__))
    #     # file_path = os.path.join(base_path, "search_config/config_search_catalog.json")
    #     output = await run_func_for_qa_dip(
    ...
    # 分析问答型搜索的托底算法
    async def asset_search_for_qa_dip_catalog(self):
    # async def run_func_catalog_for_qa_dip(self):
        '''数据目录版认知搜索列表的算法入口函数'''
        # base_path = os.path.dirname(os.path.abspath(__file__))
        # file_path = os.path.join(base_path, "search_config/config_search_catalog.json")
        # output = await self.run_func_for_qa_dip_new(
        output = await self.asset_search_for_qa_dip(
            file_path=catalog_config_path,
            data_type='datacatalog'
        )
        logger.debug(f"处理前搜索输出：{json.dumps(output, indent=4, ensure_ascii=False)}\n")
    ...
    # 未完成
    async def asset_search_for_qa_dip(self,file_path, data_type):
        # async def run_func_for_qa_dip_new(self,file_path, data_type):
        """
        分析问答型搜索的保底方法，asset_search_for_qa_dip_resource()和 asset_search_for_qa_dip_catalog()都调用这个函数，
        根据资源版或目录版传入的不同参数来执行
        ...
        logger.info(f'self.search_params={self.search_params}')
        logger.info(f'self.search_configs={self.search_configs}')
        logger.info(f"self.search_params.query={self.search_params.query}")
        query = self.search_params.query
        if not query:
            return output
        total_start_time = time.time()
        logger.info(
            f'INPUT：\nkg_id: {self.search_params.kg_id}\nquery: {self.search_params.query}\nlimit: {self.search_params.limit}\nrequired_resource: {self.search_params.required_resource}\n')
        logger.info(
            f'INPUT：entity2service: {self.search_params.entity2service}\nstopwords: {self.search_params.stopwords}\nstop_entities: {self.search_params.stop_entities}\nfilter: {self.search_params.filter}')
    ...
    async def cognitive_search_for_qa(self, request, search_params, file_path, data_type, query_embedding, m_status):
        """
    ...
    async def cognitive_search_resource_for_qa(self, request, search_params, query_embedding, m_status):
        '''数据资源版认知搜索列表的算法入口函数'''
        # file_path是本地配置文件路径
        # base_path = os.path.dirname(os.path.abspath(__file__))
        # file_path = os.path.join(base_path, "search_config/config_search_resource.json")
        output, headers, total_time_cost, all_hits, drop_indices = await self.cognitive_search_for_qa(
            request=request,
            search_params=search_params,
            file_path=resource_config_path,
            data_type='resource',
    ...
    async def cognitive_search_catalog_for_qa(self, request, search_params, query_embedding, m_status):
        '''数据目录版认知搜索列表的算法入口函数'''
        # base_path = os.path.dirname(os.path.abspath(__file__))
        # file_path = os.path.join(base_path, "search_config/config_search_catalog.json")
        output, headers, total_time_cost, all_hits, drop_indices = await self.cognitive_search_for_qa(
