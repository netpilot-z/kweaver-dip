...
class ModelRecall(object):
    ...
    async def by_keywords(self,search_index, keywords, should_querys, **kwargs):
        ...
        # responses = []
        # n_params = len(params)
        # batch_size = n_params // 10 + 1 if n_params % 10 else n_params // 10
        # for i in range(batch_size):
        #     tmp_params = params[i*10: max((i+1)*10, n_params)]
        #     response = await ad_opensearch_connector(appid=self.appid, params=tmp_params)
        #     # 归一化得分
        #     for res in response['responses']:
        #         hits = res['hits']
        #         max_score = hits['max_score']
    ...
    async def run(self, texts, **kwargs):
        ...
        balance_weight = kwargs.get("balance_weight", dict())
        user_vector = kwargs.get("user_vector", True)
        should_querys = []
        ...
        if keywords:
            query_responses = await self.by_keywords(search_index=self.index,
                                                     ...
                                                     params=kwargs.get('params', {"min_score": 1.0}))
        ...
        for idx, text in enumerate(texts):
            ...
            for _id in _ids:
                if _id in query_res and _id in vector_res:
                    ...
                    if balance_weight:
                        item['_score'] = query_score*balance_weight["query"] + vector_score*balance_weight["vector"]
                    elif not user_vector:
                        item['_score'] = query_score
                    else:
                    ...
                    query_score = query_res[_id]['_score']
                    if balance_weight:
                        item["_score"] = query_score*balance_weight["query"]
                    ...
                    vector_score = vector_res[_id]['_score']
                    if balance_weight:
                        item["_score"] = vector_score*balance_weight["vector"]
                    elif not user_vector:
                        item['_score'] = 0
                    item['query_score'] = 0
                    item['vector_score'] = vector_score
...
if __name__ == '__main__':
    import asyncio
    from app.cores.recommend.configs import recall_table_params



    #
    print('*********************')
    recall = ModelRecall(params=recall_table_params)
    # terms_musts = {'table_kind': ['1'], 'business_model_type': ['software', 'data']}
    # terms_musts = {'table_kind': ['2'], 'business_model_type': ['software']}
    # terms_musts = {'table_kind': ['1', '3'], 'business_model_type': ['data']}
    # terms_musts = {'table_kind': ['3'], 'business_model_type': ['software', 'data']}
    terms_musts = {'table_kind': ['3', '4'], 'business_model_type': ['software']}
    asyncio.run(recall.run(space_name='u61edf996499a11ef87961a9bda268c66', texts=['知识产权'], terms_musts=terms_musts))

    # print('*********************')
    # recall = ModelRecall(params=recall_flow_params)
    # # terms_musts = {'business_model_type': ['software', 'data']}
    # # terms_musts = {'business_model_type': ['software']}
    # terms_musts = {'business_model_type': ['data']}
    # asyncio.run(recall.run(space_name='u61edf996499a11ef87961a9bda268c66', texts=['流程'], terms_musts=terms_musts))
