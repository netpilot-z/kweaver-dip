...
import (
	...
	"github.com/kweaver-dip/gbkn/consumer/internal/config"
	"github.com/kweaver-dip/gbkn/consumer/internal/handler"
	"github.com/kweaver-dip/gbkn/consumer/internal/logic"
...
func main() {
	...
	topics := c.KafkaTopics()
	if len(topics) == 0 {
		logx.Error("未配置 Kafka 主题：请在 MQ.Kafka 下设置 topics 或 topic")
		os.Exit(1)
	}
	...
	logx.Infof("Kafka消费者已启动，订阅主题: %v, 消费者组: %s", topics, c.MQ.Kafka.Group)
