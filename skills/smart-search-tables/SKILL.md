...
在数据分析员工体系中，本 skill **宜由** [smart-data-analysis](../smart-data-orchestrate/SKILL.md) **总入口完成意图与 KN 编排后再进入执行**。
...
```text
找表进度：
- [ ] 1. query_object_instance：在元数据 KN 下检索，得到表/视图候选与部门/主题线索（见 query-object-instance reference）
- [ ] 2. department_duty_query：针对线索中的部门与主题构造 query，在职责 KN 下检索（见 department-duty-query reference）
- [ ] 3. 总结：最终结果必须展示候选表（以表业务名 `business_name` 为主，且必须使用完整全称，不得截断或缩写；同时补充表技术名 `technical_name`）+ 职责要点；不暴露 token 与完整调试 URL
```
...
3. **第 2 步的职责 `query`** 必须能由第 1 步结果 **派生**；若第 1 步无部门线索，则用用户原问题中的部门/组织词，或 **简要反问** 后再调职责查询。
...
由 [smart-data-analysis](../smart-data-orchestrate/SKILL.md) 路由到本 skill 时，主意图为 **找表/定位**；若上下文已有 `kn_id_find_table`，仅当其可确认是元数据知识网络时，才可用于第 1 步 `query_object_instance` 的 `kn_id`。
